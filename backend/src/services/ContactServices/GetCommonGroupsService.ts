...
const GetCommonGroupsService = async ({
  ...
  try {
    ...
    console.warn(`[FALLBACK] Error al obtener contacto/grupos comunes de WhatsApp: ${error.message || error}`);
    commonGroups = [];
  }
  ...
  for (const group of commonGroups) {
    ...
    try {
      ...
      groupsInfo.push({
        id: groupId,
        name: groupChat.name || "Sin nombre",
