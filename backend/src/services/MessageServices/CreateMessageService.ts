...
const CreateMessageService = async ({
  ...
  try {
    ...
    if (existingMessage) {
      console.info("Mensaje ya existe en la base de datos, actualizando:", messageData.id);
    } else {
    ...
    if (!messageData.fromMe && message.ticket?.contact) {
      try {
        ...
        console.error("Error al actualizar lastContactAt:", error);
      }
    }
    ...
    try {
      ...
      console.error("Error al recargar ticket antes de emitir appMessage:", reloadError);
    }
    ...
    console.error("Error al crear/actualizar mensaje:", error);
    throw error;
  }
};
