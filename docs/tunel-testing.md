...
```bash
ssh -N -R 2222:localhost:22 deploy@186.182.214.12
```
...
El `-R 2222:localhost:22` reenvía al `localhost:22` **de la máquina donde corre el
...
Con el túnel arriba, entrar a testing vía prometheus:
...
Si un túnel muere sin cerrarse, su sesión sshd en prometheus queda viva y el puerto 2222
...
```bash
ssh deploy@prometheus "fuser -k 2222/tcp"
```
